...
def hero_section(comp, meta, bus):
    ...
    bm = INDUSTRY_BENCHMARK.get(region, INDUSTRY_BENCHMARK[DEFAULT_BENCHMARK_REGION])
    stats = [
        ("调研人数", str(meta["total_respondents"]), _benchmark_note(meta, "total_pct", meta["total_respondents"]), "#2563eb", ""),
...
def manager_hero(dept, meta, bu, l2, comp, bu_name, l2_name):
    ...
    stats = [
        ("调研人数", str(dept["n"]), f"占员工总数 {n_pct}%", "#2563eb", ""),
        ("敬业员工", f'{eng["engaged_pct"]:.0f}%', f'vs 公司 {ceng["engaged_pct"]:.0f}%',
         "#10b981", "个人均分 ≥ 4.0 的员工视为敬业（Q12 口径）"),
        ("怠工员工", f'{eng["disengaged_pct"]:.0f}%', f'vs 公司 {ceng["disengaged_pct"]:.0f}%',
         "#ef4444" if eng["disengaged_pct"] >= 20 else "#f59e0b",
...
def render_vp(data, bu_name, insights):
    ...
    body += (f"<div class='vp-wide'><div class='ct'><span class='dot'></span>二级部门维度数据对比 "
...
def render_manager_l2(data, bu_name, l2_name, insights):
    ...
    stats = [
        ("调研人数", str(l2["n"]), f"占员工总数 {n_pct}%", "#2563eb", ""),
...
def render_manager(data, bu_name, l2_name, dept_name, insights):
    ...
    # 最后：优先改善项 + 30天建议 上下通栏拆开展示，确保整齐对齐、不留白
    rc_html = rootcause_conversation(dept, meta, insights)
